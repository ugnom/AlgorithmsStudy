...
b. 
	0からiまでの項についてのソートの完了を証明する必要がある。ループ不変式としてそのソートを検証する
	初期条件：i = 0。よってソート済み配列はか空のため明らかに成り立つ
	ループ内条件：0からiまでのリストの要素がソート済みと仮定する。リストの右端からi+1まで2要素ずつを比較し小さいほうの値を
	左に移してゆく。最終的に右端からi+1までの最小値がi+1に入ることになる。これにより0からi+1までの要素がソートされたことになる
	終了条件： 右端までを操作して終了。この時0-nまでのすべての要素がソートされている。
'''
def bubble_sort (input) :
	for i in range(len(input)-1) :
		for j in reversed(range(i+1, len(input)-1)) :
